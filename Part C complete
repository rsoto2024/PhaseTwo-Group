...
inner_loop:
    LDR R3, [R0, R2, LSL #2]     @ Load element at index R2 (word size)
    LDR R4, [R0, R2, LSL #2]     @ Load previous element (fixed syntax)

    CMP R3, R4
    BHS no_swap

    STR R3, [R0, R2, LSL #2]     @ Swap elements
    STR R4, [R0, R2, LSL #2]
...
array_rotate:
    ...
    LDR R2, [R0, R1, LSL #2]  @ Load last element (fixed offset)
